...
LANG_NOTES = {
    ...
    "am": "Replaces a previous build that audited at 118.9% WER. Now uses the WindyProLabs Amharic port + `forced_decoder_ids` inference. Verified at WER 26.2% / CER 11.2% / script-match 100% on 50-sample FLEURS am_et — OK tier. Always pass `forced_decoder_ids = processor.get_decoder_prompt_ids(language='amharic', task='transcribe')` to `model.generate()`.",
    "ps": "**EXCELLENT tier when used correctly.** Derived from `ihanif/whisper-medium-pashto`. Verified at WER 5.3% / CER 3.2% / script-match 99.2% on 50-sample FLEURS ps_af *when inference uses* `forced_decoder_ids` (passed explicitly to `model.generate()` via `processor.get_decoder_prompt_ids(language='pashto', task='transcribe')`). With the convenience `language=` kwarg the model can silently drop the Pashto token and hallucinate English script on ~30% of samples (53.7% WER artifact). Always force the decoder prompt for Pashto inference.",
